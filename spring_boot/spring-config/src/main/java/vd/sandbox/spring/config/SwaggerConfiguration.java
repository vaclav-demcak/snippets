...
public class SwaggerConfiguration {
  ...
  ApiInfo apiInfo() {
    return new ApiInfoBuilder()
        .title("Spring Swagger Configuration Sample API Documentation")
        .description("Propagate configuration SpringBoot REST endpoint.")
        .license("")
  ...
  public Docket customImplementation(ServletContext servletContext, @Value("${base-path:}") String basePath) {
    return new Docket(DocumentationType.SWAGGER_2)
        .select()
        .apis(RequestHandlerSelectors.basePackage("vd.sandbox.spring.config.api"))
        .build()
