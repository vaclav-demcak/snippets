package vd.samples.springboot.statemachine.config;
...
public class AppSwaggerDocConfig {
    ...
    @Bean
    public GroupedOpenApi origApi() {
        return GroupedOpenApi.builder().group("Original API").pathsToMatch("/orig/**").build();
    }


    private Info getInfo() {
        return new Info().title("Spring State-Machine Server API")
                .description("Swagger GUI for Spring State-Machine Server API services")
    ...
    private License getLicense() {
        return new License()
                .name("ↄ⃝  Copyleft 2024 VD Slovakia. All bugs reserved; ")
                .url("");
    }
}
